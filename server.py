...
@app.route('/lowlevel/<method>', methods=['POST'])
def post_lowlevel(method):
  ...
  if method not in Action.METHOD_START and method not in Action.METHOD_STOP:
    ...
    if method == Action.ACTION_EXECUTE:
      ret['result'] = ep.execute(j['arguments'])
      if ret['result'] == -1:
        ret['result'] = None
      ...
      ret['result'] = ep.kill_app(j['arguments'][0])
    elif method == Action.ACTION_KILL_PID:
      ret['result'] = ep.kill_pid(j['arguments'][0])
    elif method == Action.ACTION_CLOSE_WINDOW:
      ret['result'] = ep.close_window(j['arguments'][0])
  result = jsonify(ret)
  result.status_code = 200
  return result
